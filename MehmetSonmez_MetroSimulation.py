...
class MetroAgi:
    ...
    def en_az_aktarma_bul(self, baslangic_id: str, hedef_id: str) -> Optional[List[Istasyon]]:
        ...
        """
        #Başlangıç yada hedef istasyon listede yoksa null değer döndür
        ...
        ziyaret_edildi = set()             #ziyaret_edilen istasyonlar
        kuyruk = deque([(baslangic, [baslangic])])    #kuyruk oluştur

        #kuyrukta eleman olduğu sürece devam et
        while kuyruk:
            mevcut, yol = kuyruk.popleft() #kuyruktan ilk elemanı al

            #hedef istasyona gelindi ise yolu döndür
            if mevcut == hedef:     
                return yol

            #istasyon edilen listesinde ise sonraki elemana devam et
            if mevcut in ziyaret_edildi:  
                continue

            #istasyonu ziyaret edilenler listesine ekle
            ziyaret_edildi.add(mevcut)

            #istasyonun komşularını da kuyruğa ekle
    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        # Ziyaret edilen istasyonlar ve en kısa sürede ziyaret etme kayıtları
        ziyaret_edildi = {}

        while pq:
            toplam_sure, _, mevcut, rota = heapq.heappop(pq)  # En düşük süreli istasyonu çıkar

            # Eğer hedef istasyona ulaştıysak sonucu döndür
            if mevcut == hedef:
              return rota, toplam_sure

            # Daha önce ziyaret edilmemiş veya daha kısa sürede ulaşılan bir istasyon mu?
            if mevcut.idx in ziyaret_edildi and ziyaret_edildi[mevcut.idx] <= toplam_sure:
                continue  # Önceki ziyaret daha hızlıysa bunu atla

            ziyaret_edildi[mevcut.idx] = toplam_sure  # Mevcut istasyona ulaşılan en kısa süreyi güncelle

            # Komşu istasyonları kontrol et
            for komsu, sure in mevcut.komsular:
                yeni_sure = toplam_sure + sure  # Yeni toplam süre
                heapq.heappush(pq, (yeni_sure, id(komsu), komsu, rota + [komsu]))  # Yeni yolu kuyruğa ekle

        return None  # Eğer hedefe ulaşılamadıysa
